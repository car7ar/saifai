...
async function dbSave(key, val) {
    ...
    // 2. Cloud Sync (Supabase) - Only for major content
    if (!sb || !['items', 'cats', 'pages'].includes(key)) return;
    try {
        if (state.user?.role !== 'admin') return; // Security: Only admin syncs content

        if (key === 'items') {
            await sb.from('items').upsert(val);
        } else if (key === 'cats') {
            const catObjs = val.map(c => ({ name: c }));
            await sb.from('categories').upsert(catObjs, { onConflict: 'name' });
        } else if (key === 'pages') {
            await sb.from('pages').upsert(val);
        }
    } catch (e) { console.error('Cloud Sync Error:', e); }
}

async function dbLoad(key) {
    // 1. Try Cloud for content
    if (sb && ['items', 'cats', 'pages'].includes(key)) {
        try {
            const table = key === 'cats' ? 'categories' : key;
            const { data, error } = await sb.from(table).select('*');
            if (!error && data && data.length > 0) {
                if (key === 'cats') return data.map(d => d.name);
                return data;
            }
        } catch (e) { console.warn(`Cloud Load Failed for ${key}`); }
    }
...
// ─── Supabase Config (Live Connection) ───
const SUPABASE_URL = 'https://xlpiuihfcdvfphwyqjyx.supabase.co';
const SUPABASE_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InhscGl1aWhmY2R2ZnBod3lxanl4Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3Nzk0NzM4MDgsImV4cCI6MjA5NTA0OTgwOH0.yX5qez9Sy6d5jyhCrqwbg8KukBWtJdo2yAoJ11dGphY';
let sb = null;
try {
...
async function init() {
    ...
    if (savedItems) _state.items = savedItems;
    if (savedCats)  _state.cats  = savedCats;
    else            _state.cats  = [];
    if (savedFavs)  _state.favs  = savedFavs;
    if (savedPages) _state.pages = savedPages;
...
function renderGrid() {
    ...
    if (_state.loading) {
        box.innerHTML = Array(8).fill(0).map(() => `
            <div class="card" style="pointer-events:none;">
                <div style="width:100%; aspect-ratio:1; background:linear-gradient(90deg,var(--bord) 25%,var(--bg) 50%,var(--bord) 75%); background-size:200% 100%; animation:shimmer 1.4s infinite; border-radius:14px;"></div>
                <div class="card-body">
                    <div style="height:14px; background:var(--bord); border-radius:8px; margin-bottom:8px; animation:shimmer 1.4s infinite;"></div>
                    <div style="height:10px; background:var(--bord); border-radius:8px; width:60%; animation:shimmer 1.4s infinite;"></div>
                </div>
            </div>
        `).join('');
        return;
...
const ux = {
    ...
    updateNav: (hash) => {
        ...
        if (hash === '#home' || hash === '') document.getElementById('nav-home')?.classList.add('active');
        if (hash === '#cats') document.getElementById('nav-cats')?.classList.add('active');
        if (hash === '#favs') document.getElementById('nav-favs')?.classList.add('active');
        if (hash === '#profile') document.getElementById('nav-profile')?.classList.add('active');
        if (hash.startsWith('#page/')) {
            const pageId = hash.split('/')[1];
            document.getElementById('nav-page-' + pageId)?.classList.add('active');
        }

        // Mobile Bottom Nav Highlights
        const mBtns = document.querySelectorAll('.m-nav-btn');
        mBtns.forEach(el => el.classList.remove('active'));
        if (hash === '#home' || hash === '') mBtns[0]?.classList.add('active');
        if (hash === '#cats') mBtns[1]?.classList.add('active');
        if (hash === '#favs' || hash.startsWith('#page/')) mBtns[2]?.classList.add('active'); 
        if (hash === '#profile') mBtns[3]?.classList.add('active');
    },
    togglePreview: () => {
        const p = document.getElementById('side-preview');
        const btn = document.getElementById('toggle-p-btn');
        if (!p) return;
        const isOff = p.style.display === 'none' || !p.classList.contains('active');
        if (isOff) {
            p.classList.add('active');
            btn.innerHTML = `<i data-lucide="eye-off" size="18"></i> <span>إخفاء المعاينة</span>`;
        } else {
            p.classList.remove('active');
            btn.innerHTML = `<i data-lucide="eye" size="18"></i> <span>إظهار المعاينة المباشرة</span>`;
        }
        if(window.lucide) lucide.createIcons();
    },
    openMenu: () => {
        const mCatList = document.getElementById('m-cat-list');
        if (mCatList) {
            mCatList.innerHTML = state.cats.map(c => `
                <div class="nav-item ${state.filter === c ? 'active' : ''}" onclick="actions.setFilter('${c}'); ux.closeM();">
                    <i data-lucide="hash"></i> <span>${c}</span>
                </div>
            `).join('');
        }
        const mPageList = document.getElementById('m-page-list');
        if (mPageList) {
            let html = `<div class="nav-item ${state.filter === 'favs' ? 'active' : ''}" onclick="actions.filterFavs(); ux.closeM();"><i data-lucide="heart"></i> <span>الأعمال المفضلة</span></div>`;
            html += state.pages.map(p => `
                <div class="nav-item ${state.filter === 'page_'+p.id ? 'active' : ''}" onclick="actions.viewPage('${p.id}'); ux.closeM();">
                    <i data-lucide="file-text"></i> <span>${p.title}</span>
                </div>
            `).join('');
            if (state.user?.role === 'admin') {
                html += `<div class="nav-item" style="color:var(--p); border-top:1px solid var(--bord); margin-top:1.5rem; padding-top:1rem;" onclick="actions.viewAdmin(); ux.closeM();"><i data-lucide="layout-dashboard"></i> <span>لوحة التحكم</span></div>`;
            }
            mPageList.innerHTML = html;
        }
        if(window.lucide) lucide.createIcons();
            ...
            grid.innerHTML = `
                <div class="profile-page" style="grid-column: 1/-1; animation: focusIn .5s ease; gap:20px;">
                    ...
                    <div style="max-width:600px; margin: 30px auto 0; width:100%;">
                        ...
                        ${user.role === 'admin' ? `
                        <!-- Admin Admin Button -->
                        <button onclick="actions.viewAdmin()" style="width:100%; height:55px; background:linear-gradient(135deg,var(--p),#4f46e5); color:white; border:none; border-radius:16px; font-size:0.95rem; font-weight:900; cursor:pointer; display:flex; align-items:center; justify-content:center; gap:10px; margin-bottom:20px; box-shadow:0 8px 20px rgba(99,102,241,0.3);">
                            <i data-lucide="layout-dashboard" size="20"></i> لوحة التحكم • Admin Panel
                        </button>
                        ` : ''}
